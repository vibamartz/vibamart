...
import {
  ...
  ExternalLink, Search, Filter, ShoppingBag, CheckCircle2, XCircle, Share2
} from 'lucide-react';
              ...
              >
                ...
                <div className="flex gap-2 pt-1" onClick={(e) => e.stopPropagation()}>
                  {(coupon.remainingQuantity ?? 0) <= 0 || !coupon.active || (user && rewardOrders.some(ro => ro.couponId === coupon.id && ro.userId === user.uid)) ? (
                    <button
                      disabled
                      className="flex-1 py-2.5 bg-rose-100 text-rose-600 rounded-xl text-xs font-black cursor-not-allowed flex items-center justify-center gap-1"
                    >
                      Expired/Used
                    </button>
                  ...
                  <button
                    ...
                    className="p-2.5 bg-gray-50 border border-gray-200 text-gray-700 rounded-xl text-xs font-bold flex items-center justify-center shrink-0 active:scale-95 transition-transform cursor-pointer"
                    title="Share coupon"
                  >
