           ...
           {selectedCategories.length > 0 && (
             <div className="flex gap-6 overflow-x-auto no-scrollbar py-8 mb-4">
               {selectedCategories.flatMap(catId => 
                 ...
                 >
                   <div className={`w-16 h-16 rounded-full overflow-hidden border-2 transition-all p-0.5 bg-white shadow-sm flex-shrink-0 ${
                   ...
                   <span className={`text-[10px] font-black uppercase tracking-tight text-center whitespace-wrap max-w-[80px] leading-tight transition-colors ${
                     selectedSubCategories.includes(sub.id) ? 'text-primary' : 'text-gray-500 group-hover:text-gray-900'
                   }`}>
                     {sub.name}
                   </span>
                 </button>
                 ...
                 <FilterSection title="Discount">
                   {[40, 30, 10].map(d => (
                     <FilterOption 
                      key={d}
                      label={`${d}% or more`} 
                      count={allProducts.filter(p => p.discountPrice && ((p.price - p.discountPrice) / p.price) * 100 >= d).length} 
             ...
             >
                ...
                <div className="space-y-8">
                   <FilterSection title="Availability">
                     <FilterOption 
                      label="In Stock Only" 
                      count={allProducts.filter(p => p.stock > 0).length} 
                      checked={onlyInStock}
